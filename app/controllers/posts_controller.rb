class PostsController < ApplicationController
  ...
  before_action :set_markdown_content, only: %i[ show edit ]
  ...
  def show
    ...
    #     @markdown_content = @post.markdown_to_html
    #   end
  ...
  # Set the markdown content for the post
  def set_markdown_content
    if @post.markdown? 
      if @post.file_path.present? && File.exist?(@post.markdown_file_path)
        @markdown_content = @post.markdown_to_html
      else
  ...
  def post_params
    params.expect(post: [ 
        # Generic stuff
        :title, :slug, :status, :category, :tldr, :image_path, :source_type, 
        # Database stuff
        :content, 
        # Markdown stuff
